...
public class Parser {
	...
	public void advance(){
		...
		currentCommand = currentCommand.toLowerCase().replaceAll("(\\s|//.*)", "");
		
		Matcher arithmeticCmdMatcher = Pattern.compile("^(add|sub|neg|eq|gt|lt|and|or|not)$").matcher(currentCommand);
		Matcher memAccessCmdMatcher = Pattern.compile("^(pop|push)(argument|local|static|constant|this|that|pointer|temp)(\\d+)$").matcher(currentCommand);
		Matcher programFlowCmdMatcher = Pattern.compile("^(label|goto|if\\-goto)(.+)$").matcher(currentCommand);
		Matcher functionCmdMatcher = Pattern.compile("^(function|call)(\\D+)(\\d+)$").matcher(currentCommand);
		...
		if(arithmeticCmdMatcher.matches()){
			...
			if(programFlowCmdMatcher.group(1) == "if-goto"){
				commandType = "C_IF";
			}else{
				//C_LABEL, C_GOTO
